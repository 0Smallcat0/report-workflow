...
#: Arms whose body is checked against a hash in their own header. The tool arm
#: is not one: it is regenerated by `scripts/record_tool_arm.py` whenever the
#: pipeline changes what reaches the deliverable, so a hash on it would be
#: stale by design rather than a signal.
FROZEN_ARMS = ("hand", "llm_direct")
...
def check_archive() -> list[str]:
    ...
    """
    issues: list[str] = [
        problem for arm in FROZEN_ARMS for problem in check_frozen_body(ARMS[arm])
    ]
