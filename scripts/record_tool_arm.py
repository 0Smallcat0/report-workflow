...
CLAIMS = [
    ...
    ("es2", [E_RVP, E_PRICE],
     ...
     "結論是值得進入，切點是 200–500 美元這一帶，但以單一機種試水而不是鋪產品線。"
    ...
    ("r1", [E_RVP, E_PRICE],
     ...
     "建議的切點是 200–500 這一帶：口碑側平均星等 4.66，掛牌側累積評論中位數 327.00 是全表最高，"
     "而該帶掛牌只有 53 件、佔 9.74%。4.66 來自評論檔，327.00 與 53 件來自掛牌檔，是兩個獨立的量；"
     "4.90% 的負評率與 4.66 出自同一批 102 則評論，只是同一個訊號的第二種寫法，不另計為佐證。"
     "本報告也不把切點限定在某個品類：品類表與價格帶表沒有共同欄位，"
     "攝影在這一帶佔多少，這份資料沒有算過。"),
