...
def _initial_observed_hashes(run_dir: Path, first_stage: str) -> dict[str, str | None]:
    ...
    for rel_path, digest in snapshot.items():
        if digest is not None and not _matches(rel_path, allowed):
...
def _new_manifest(state: ReportState, run_dir: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "observed_hashes": _initial_observed_hashes(run_dir, first_stage),
...
def _record_scope_violation(
    ...
    entry["last_error"] = {
        "message": "Author-owned artifacts changed outside the current stage write scope.",
